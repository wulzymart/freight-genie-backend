...
import {OrderHistory} from "../../../../../db/entities/vendor/order-history.entity.js";
...
export async function addOrder(
    ...
    const historyRepo = vendorDataSource.getRepository(OrderHistory);
...
export async function setPayOnDelivery(
    ...
    const id = request.params.id;
    const orderRepo = request.vendorDataSource!.getRepository(Order);
    const order = await orderRepo.findOneBy({id});
    if (!order) return reply.status(404).send({success: false, message: "Order not found"});
    if (![OrderStatus.PENDING, OrderStatus.ACCEPTED].includes(order.status) || order.paymentInfo)
        return reply.status(400).send({
            success: false,
            message: "Payment on delivery cannot be set, Order has either been paid for or shipped"
        });
    order.status = OrderStatus.ACCEPTED
    order.trackingInfo = order.trackingInfo ? order.trackingInfo : [{
        info: 'Order accepted at origin station, will be shipped soon',
        time: new Date()
    }];
...
export async function payOrder(
    request: FastifyRequest<{
        Params: { id: string },
        Body: { paymentType: PaymentType, receiptInfo: string, amount: number }
    }>,
    reply: FastifyReply
) {
    ...
    const id = request.params.id;
    const user = request.user as User;
    if (!id || !amount || !paymentType || !receiptInfo)
        return reply
            .status(400)
            .send({success: false, message: "invalid action"});
    return await vendorDataSource.transaction(async (manager) => {
        const orderRepo = manager.getRepository(Order);
        const order = await orderRepo.findOneBy({id});
        if (!order) throw new Error("Order not found", {cause: 404});
        if (order.paymentInfo)
            throw new Error("Order has either been already paid for", {cause: 400});
        console.log('total=', order.price.total, 'amount=', amount)
        if (+order.price.total !== +amount) throw new Error("Amount not same as order price", {cause: 400});
        const orderPaymentRepo = manager.getRepository(OrderPayment);
        const receiptRepo = manager.getRepository(Receipt);
        const historyRepo = manager.getRepository(OrderHistory);
        ...
        if (!customer) throw new Error('Order Customer not found', {cause: 404});
        ...
        if (paymentType === PaymentType.WALLET) {
            ...
            const walletPayment = walletPayRepo.create({
                amount,
                corporateCustomerId: customer?.corporateCustomer.id,
                orderId: order.id,
                processedById: user.staff.officePersonnelInfo?.id!
            })
            await walletPayment.save()
            order.paymentInfo.walletPaymentId = walletPayment.id;
        } else {
            const orderReceipt = receiptRepo.create({
                orderId: order.id,
                amount,
                receiptInfo,
                receiptType: ReceiptType.ORDER_PAYMENT,
                customerId: order.customerId,
                corporateCustomerId: customer?.corporateCustomer?.id || undefined
            })
            await orderReceipt.save()
            order.paymentInfo.receiptId = orderReceipt.id;
        }
        await order.paymentInfo.save()
        const hx = await historyRepo.create({info: 'Payment for order made', performedById: user.staff.id}).save();
        order.history.push(hx);
