...
export async function getRoutingORS(coordinates:Coordinate[]) {
  ...
  const {data} = await axios.post('https://api.openrouteservice.org/v2/directions/driving-hgv/geojson', {coordinates,instructions:true,units:"km"}, {
    headers: {
      "Content-Type": "application/json",
      "Accept": 'application/json, application/geo+json, application/gpx+xml, img/png; charset=utf-8',
      Authorization: process.env.ORS_TOKEN
...
export async function getRoutingOSRM(coordinates:Coordinate[]) {
  ...
  const url = `http://router.project-osrm.org/route/v1/driving/${formatedCordinates}?overview=full&geometries=geojson`
  const {data} = await axios.get(url)
